...
static ssize_t _temp_handler(coap_pkt_t *pkt, uint8_t *buf, size_t len, void *context)
{
    (void)context;
    char answer[24] ;
    sprintf(answer, "temp is %2i.%02i°C\n ",temp_abs,temp); 
    return coap_reply_simple(pkt, COAP_CODE_205, buf, len,
            COAP_FORMAT_TEXT, (uint8_t*)answer, strlen(answer));
}


static ssize_t _pressure_handler(coap_pkt_t *pkt, uint8_t *buf, size_t len, void *context)
{
    (void)context;
    char answer[24] ;
    sprintf(answer, "pressure is %ihPa",pres); 
    return coap_reply_simple(pkt, COAP_CODE_205, buf, len,
            COAP_FORMAT_TEXT, (uint8_t*)answer, strlen(answer));
}
...
const coap_resource_t coap_resources[] = {
    ...
    { "/pres", COAP_GET, _pressure_handler, NULL },
};
